...
class Controler():
    ...
    def create_tournament(self):
        ...
        description_tournament = self.view.ask_description_tournament()
        # liste des joueurs du tournoi
        list_of_player = self.view.ask_list_of_player_tournament()
        for i in list_of_player:
            joueur = player.Joueur(name=i["name"],
                                   second_name=i["second_name"],
                                   birthday=i["birthday"], sex=i["sex"])
            # liste des joueurs présents dans le controleur
            self.player.append(joueur)
        # après la liste des joueurs place à la liste
        # des round pour le premier tour
        for i in range(4):
            # on créee des paires de match pour le premier round
            m = match.Match(self.player[i], self.player[i+4])
            # chaque match est ajouté à la liste round1 donc à la
            # fin ici on a une liste avec 4 matchs
        ...
        self.tour = match.Tournament(name=name_tournament,
                                     place=place_tournament,
                                     description=description_tournament,
                                     list_of_round=[],
                                     list_of_player=self.player)

    def sort_player_by_point(self):
        """cete fonction permet de trier la liste nomée
        player des joueurs selon leurs points cumulés
        si deux joueurs ont le meme nombre de point alors
        ils sont triés en fonction de leur classement"""
        i = 0
    ...
    def verify_match(self, match):
        """cette fonction prend en paramètre un match et verifie
        si ce match a déjà été joué
        dans la liste de match dejà joués list_of_match_play et
        retourne TRUE si oui et FALSE sinon"""
        for i in self.list_of_match_play:
            if (i.player_list[0].family_name == match.player_list[0].family_name and
                    i.player_list[1].family_name == match.player_list[1].family_name):
                return True
            if (i.player_list[0].family_name == match.player_list[1].family_name and
                    i.player_list[1].family_name == match.player_list[0].family_name):
                return True
        return False

    def generate_next_round(self):
        """cette fonction permet de générer un nouveau round avec
        une politique de génération différent du premier
        ...
        while i < len(self.player)-1:
            ...
            i = i+2
            # il faut controler si ce match existe dans la liste
            # des match deja joués "list_of_match_play"
    ...
    def type_result(self):
        """on recupère le score de chaque joueur à partir
        du round 2 à 4 qu'on met dans dans l'objet Match
        de la liste round1 ensuit on increment l'attribut
        point de chaque joueur
        avec son score lors de ce match"""
        j = 0
        for i in self.round1:
            result = self.view.ask_result(i.player_list[0].family_name,
                                          i.player_list[1].family_name)
            i.score_list = result
            # on doit parcourir la    liste player et trouver
            # chaque joueur et incremente son nombre de point
            find = False
            j = 0
    ...
    def show_state(self):
        ...
        self.view.show_infos("MERCI D'ETRE PASSÉ ET D'AVOIR PARTICIPÉ À "
                             "NOTRE TOURNOI RENDEZ LA PROCHAINE FOIS")
